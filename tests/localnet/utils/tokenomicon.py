...
def admin_funding(config: dict, num_calls: int) -> None:
    ...
    cmd = f"btcli wallet faucet --wallet.name {admin_wallet.get("wallet_name")} -p ./wallets --processors {str(ceil(psutil.cpu_count()*.80))} --max-successes {str(num_calls)} --no-prompt --subtensor.chain_endpoint ws://127.0.0.1:{rpc_port}"
...
def register_subnet(wallet_name, hotkey_name, wallet_path, rpc_port):
    cmd = f"btcli subnet create --wallet-name {wallet_name} --wallet-hotkey {hotkey_name} -p {wallet_path} --no_prompt --subtensor.chain_endpoint ws://127.0.0.1:{rpc_port}"
    subprocess.run(cmd, shell=True, check=True)
